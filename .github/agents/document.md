...
name: Document
description: >
  Verifies that code changes satisfy the initial requirements (from doc/pr-log),
  and updates/creates doc/SERVICE_DOC.md to document the codebase entry point for
...
2. Create or update `doc/SERVICE_DOC.md` as the developer-friendly entry point to the codebase.
...
| `doc/pr-log/<ticket_id>.md` | The requirement log — your source of truth for verification. |
| `src/main/java/org/example/` | The implemented code from Phase 2. |
| `doc/SERVICE_DOC.md` | (Optional) Existing repository documentation. Create if not present. |
...
Read the requirement log (`doc/pr-log/<ticket_id>.md`) and check that the code
...
### Step 2 · Update or Create doc/SERVICE_DOC.md

Check if `doc/SERVICE_DOC.md` exists in the repository:

#### Case A: doc/SERVICE_DOC.md Does Not Exist
...
#### Case B: doc/SERVICE_DOC.md Already Exists
...
### General Guidelines for doc/SERVICE_DOC.md
...
Before finalizing `doc/SERVICE_DOC.md`, identify areas where clarity would help future
developers:
...
List your questions and wait for the user to answer. Update `doc/SERVICE_DOC.md` based
on the answers.

After every clarification, append:
> 📍 **Current phase: Document** — ready to move to Pull Request once `doc/SERVICE_DOC.md`
...
1. Re-read `doc/SERVICE_DOC.md` to ensure it is:
...
2. Update `doc/pr-log/<ticket_id>.md` with a new section if any documentation
...
### Step 6 · Handoff
On approval:
1. Ensure `doc/SERVICE_DOC.md` is final and committed (or prepared for commit).
2. Ensure `doc/pr-log/<ticket_id>.md` is up to date with any document phase notes.
...
Artefacts:
  - doc/SERVICE_DOC.md (created/updated with codebase documentation)
  - doc/pr-log/<ticket_id>.md (updated with any Document phase clarifications)
...
- **Maintain doc/SERVICE_DOC.md quality.** It is the front door for new developers;
  keep it welcoming and clear.
- **Keep documentation in sync with code.** If code changes again later, update
  doc/SERVICE_DOC.md in the next Document phase.
...
- If the user goes on a tangent (e.g., "should we add API docs?"), answer
  ...
  > 📍 **Current phase: Document** — shall we focus on finalizing doc/SERVICE_DOC.md?
